...
def generatePlayerShotsNew(filename, playerNameDict, playerID , appearances, seasons, months, quarters):
	with open(filename, 'rU') as open_file:
			...
			for row in open_reader:
				...
				if count == 0:
					...
					if int(entry_dict['game']) in appearances:
						...
						if playerID == desiredPlayer:
							...
							if entry_dict['season'] in seasons:
									...
									if entry_dict['game'][4:6] in months:
										...
										playerNameDict['shot']['Games Played:'] = playerNameDict['shot']['Games Played:']+1
...
def get_player_DB(playerName, seasons,months, quarters):
	...
	generatePlayerShotsNew('complete_data.csv', playerNameDict, playerID , appearances, seasons, months, quarters)
